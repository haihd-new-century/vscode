...
import { TasksTreeProvider } from './features/tasks/tasks-provider';
import { ApprovalTreeProvider } from './features/approval/approval-provider';
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  // 3. Register sidebar view providers
  chatProvider = new ChatViewProvider(context.extensionUri, apiClient, state);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(VIEW_IDS.CHAT, chatProvider),
  );

  tasksProvider = new TasksTreeProvider(apiClient);
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider(VIEW_IDS.TASKS, tasksProvider),
  );

  approvalProvider = new ApprovalTreeProvider(apiClient);
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider(VIEW_IDS.APPROVALS, approvalProvider),
  );

  collectionsProvider = new CollectionsTreeProvider(apiClient, state);
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider(VIEW_IDS.COLLECTIONS, collectionsProvider),
  );

  searchProvider = new SearchTreeProvider(apiClient, state);
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider(VIEW_IDS.SEARCH, searchProvider),
  );

  unityFeature = registerUnitySessionsView(context, apiClient);
