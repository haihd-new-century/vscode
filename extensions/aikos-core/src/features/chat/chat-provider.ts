...
import { UnitySessionsProvider } from '../unity/unity-sessions-provider';
import { UnitySessionState } from '../unity/unity-state';
import { parseUnityMention } from '../unity/unity-chat-mention';

export class ChatViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = VIEW_IDS.CHAT;
  private webviewView?: vscode.WebviewView;
  private conversationId?: string;
  private unityProvider?: UnitySessionsProvider;
  private unityState?: UnitySessionState;

  /** Late-bound after the Unity sidebar registers itself. */
  attachUnity(provider: UnitySessionsProvider, state: UnitySessionState): void {
    this.unityProvider = provider;
    this.unityState = state;
  }
  ...
  private async handleChat(query: string): Promise<void> {
    ...
    // Resolve @unity[:slug] mentions → pinned session for downstream tool routing.
    let unitySessionId: string | undefined;
    if (this.unityProvider && this.unityState) {
      const mention = parseUnityMention(query, this.unityProvider, this.unityState);
      if (mention.mentioned || mention.sessionId) {
        query = mention.query || query;
        unitySessionId = mention.sessionId;
        if (mention.note) {
          this.postMessage({ type: 'streamNote', content: mention.note });
        }
      }
    }
  ...
  private async showConversationHistory(): Promise<void> {
    try {
      ...
      if (selected) {
        ...
        try {
          const conversation = await this.apiClient.get<{
            id: string;
            title: string;
            messages: { role: string; content: string; createdAt: string; sources?: Citation[] }[];
          ...
          if (conversation?.messages) {
            for (const msg of conversation.messages) {
              ...
              if (msg.role === 'assistant' && msg.sources?.length) {
                this.postMessage({ type: 'streamSources', sources: msg.sources });
              }
            }
            logInfo(`Loaded ${conversation.messages.length} messages from conversation ${selected.id}`);
          }
        } catch (err) {
          logError('Failed to load conversation messages', err);
          this.postMessage({
            type: 'addMessage',
            role: 'system',
            content: 'Failed to load conversation history. Start typing to continue.',
          });
        }
      }
    } catch (err) {
